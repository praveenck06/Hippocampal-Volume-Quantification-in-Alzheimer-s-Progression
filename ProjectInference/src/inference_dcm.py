...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        ...
                        Predicted Total Volume:{inference['total']}\n \
    ...
    orig_vol = med_reshape(orig_vol, (orig_vol.shape[0], 64, 64))
    position = [0, 410, 820]
    slices_to_view =[6, 16,24]
    for  i in range(len(slices_to_view)):
            slic =slices_to_view[i]
            nd_orig = np.flip((orig_vol[slic, :, :]/np.max(orig_vol[slic, :, :]))*0xff).T.astype(np.uint8)

            # create a PIL image from numpy array
            pil_orig = Image.fromarray(nd_orig, mode="L").convert("RGBA")

            nd_pred = np.flip((pred_vol[slic, :, :]/np.max(pred_vol[slic, :, :]))*0xff).T.astype(np.uint8)
            pil_pred = Image.fromarray(nd_pred)
            mask = ImageOps.colorize(pil_pred, black = "white",mid ="blue", white="red").convert("RGBA")
            print("mask size",mask.size)
            print("image size",pil_orig.size)

            final_img = Image.blend(pil_orig, mask, 0.30)
            final_img = final_img.resize((400,400))
            pimg.paste(final_img, box=(position[i], 250)) 
...
if __name__ == "__main__":
    ...
    study_dir_list = sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True)
    for dir_name in study_dir_list:
        if "HCropVolume" in dir_name:
            study_dir =dir_name
